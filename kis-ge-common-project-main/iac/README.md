...
권장 SA: `org-foundation-tf@kis-gemini-common-prod.iam.gserviceaccount.com`
...
ops 프로젝트(`kis-gemini-common-prod`) 권한:
...
|---|---|---|---|---|
| `kis_gemini_common_prod_iac_key` | 위 SA의 JSON 키 | **File** | ✓ | ✓ |
| `GCP_TF_PROJECT` | `kis-gemini-common-prod` | Variable | ✓ | – |
...
```
gs://kis-gemini-common-prod-tfstate/lzone/org/default.tfstate
```
...
4. **outputs 확인**:
   ```bash
   cd iac
   terraform init -backend-config="bucket=kis-gemini-common-prod-tfstate" \
                  -backend-config="prefix=lzone/org"
   terraform output
   # access_policy_id = "123456789"
   # central_audit_bucket = "kis-gemini-common-prod-org-audit-logs"
   # ...
   ```
